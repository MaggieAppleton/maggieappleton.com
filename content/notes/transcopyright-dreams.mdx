...
A transclusion is linked to the original, and contains the exact same contents. Someone viewing the transclusion can easily open and view the original document. By doing this, transclusions create a single source of truth for that element, and a [[Bi-Directional Link]] so you can navigate between the original source and its instances across the web.

Now, what happens if the original source changes?

The transclusion doesn't automatically  change along with it. If transclusions were direct embeds of the original content, we'd end up with a huge pile of dead links and missing words all over the place. That's what currently happens with `<img>` tags where the `src` element points to an original source URL. If the original publisher deletes the photo, your image embed breaks along with it.

<BasicImage margin="1.6em auto 1.2em" width="620px" alt="Two documents connected by a transclusion. Transclusions are the same content displayed in more than one location, visibly linked." src="https://res.cloudinary.com/dg3gyk0gu/image/upload/v1611093848/maggieappleton.com/notes/xanadu/transclusion_5_shrink.png" />

Instead, the transclusion is more like a saved version of the original – a _cached_ snapshot of it at a moment in time. You can update that snapshot to a newer version if you want to, but it's not automatic. Even if the original author deletes it, your transcluded version stays visible.
...
<FullWidthImage margin="2em auto 1.2em" width="1100px" alt="Two documents connected by a transclusion. Transclusions are the same content displayed in more than one location, visibly linked." src="https://res.cloudinary.com/dg3gyk0gu/image/upload/v1611093847/maggieappleton.com/notes/xanadu/transclusion_4_shrink.png" />

In this idealised utopia, we also want to place value on sharing and curation as well as original creation, which means giving a small fraction of the payment to the re-publisher as well.

We should note monetisation of all this content is optional. Some websites would allow their content to be transcluded for free, while others might charge hefty fees for a few sentences. If all goes well, we'd expect the majority of content on the web to be either free or priced at very small amounts.
...
A subtler approach would be to simply choose not to display transclusion links on their webpage. The equivalent of `transclusions { display: none; }`. In this scenario they would still exist, but wouldn't be as easy to access and read.

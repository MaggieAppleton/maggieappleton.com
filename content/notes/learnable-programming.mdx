...
Bret Victor coined the term <b>Learnable Programming</b> in his <a href="http://worrydream.com/LearnableProgramming">canonical essay</a> lamenting the current state of programming education.
 ...
 Victor has a few concerns with the way most of us learn to code. Mainly that the learning environments we drop people into are poorly designed.
...
People like [Alan Kay](https://en.wikipedia.org/wiki/Alan_Kay) and [Seymour Paypert](https://en.wikipedia.org/wiki/Seymour_Papert) were the earliest champions of using intuitive visuals to teach programming. Paypert's [Logo](https://en.wikipedia.org/wiki/Logo_(programming_language)) language defined a generation of visual, turtle-based learning. Through a series of simple commands like `go(20)` and `turn(90)` a small green turtle would move around your canvas leaving a visible trail behind it.

My earliest memory of programming involved directing one of these small green amphibians around in circles.
...
One of the widely-used modern manifestation of Logo is MIT's [Scratch](https://scratch.mit.edu/) learning platform. It expanded the character range beyond just turtles, offering everything from llamas to dancing starfish. More importantly, it allows you to drag-n-drop pieces of programming logic into chains that animate a scene for you.

<ImageFrame width="900px" src="https://res.cloudinary.com/dxj9qr5gj/image/upload/v1622107452/maggieappleton.com/notes/learnable-programming/scratch_aqp3ts.png" alt="The scratch learning platform that allows you to construct clear visual chains of programming commands" showalt />

If we look past the campy, child-friendly aesthetics of Scratch, it's hard to argue this kind of visual interface isn't helpful. There is no need to memorise the syntax, it's easy to browse through the available commands, and the physical shape of each command 

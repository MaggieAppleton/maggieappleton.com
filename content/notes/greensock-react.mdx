...
React builds it's own internal "tree" of DOM nodes based off our code, and only updates the actual DOM as needed (a concept previously known as the [[React Virtual DOM]]) <Footnote idName={1}><span>The React team has gone off the term of the _virtual DOM_ and prefer to talk about the _render tree_. Potatoes, potataos. For more on how the virtual DOM works, I have an [illustrated guide](/react-vdom).</span><br /><br /><span>If you have never heard of the <b>Document Object Model</b> before, this post might be a bit over your head, but [here's](https://bitsofco.de/what-exactly-is-the-dom/) a good starting point to get caught up.</span></Footnote>
